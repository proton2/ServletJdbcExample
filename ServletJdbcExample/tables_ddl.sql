...
BEGIN
    ...
    create table if not exists dev.ddl_exceptions (
	    id bigint not null default nextval('dev.hibernate_sequence'::regclass),
	    message varchar(1024),
	    CONSTRAINT ddl_exceptions_pk PRIMARY KEY (id)
    );
    ...
    for each row execute procedure dev.set_update_time();
   	...
   	--SELECT (2 / 0); --check exception

EXCEPTION
    WHEN others THEN
    BEGIN
        insert into dev.ddl_exceptions(message) values ('Error: ' || SQLSTATE ||' '|| SQLERRM);
    END;
END $$;